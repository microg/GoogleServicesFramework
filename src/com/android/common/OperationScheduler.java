 ...
 * <p/>
 * <p/>
 * This class does not directly perform or invoke any operations, it only keeps
 * track of the schedule. Somebody else needs to call
...
public class OperationScheduler {
	...
	public static class Options {
		...
		/**
		 * Wait this long times the number of consecutive errors so far before
		 * retrying.
		 */
		public long backoffIncrementalMillis = 5000;

		/**
		 * Maximum duration of moratorium to honor. Mostly an issue for clock
		 * rollbacks.
		 */
		public long maxMoratoriumMillis = 24 * 3600 * 1000;

		/**
		 * Minimum duration after success to wait before allowing another
		 * trigger.
		 */
		...
		public String toString() {
			return String.format("OperationScheduler.Options[backoff=%.1f+%.1f max=%.1f min=%.1f period=%.1f]",
	 ...
	 *
	 * @param storage to use for recording the state of operations across
	 *                restarts/reboots
	 */
	 ...
	 * </pre>
	 * <p/>
	 * All values are times in (possibly fractional) <em>seconds</em> (not
	 * milliseconds). Omitted settings are left at whatever existing default
	 * value was passed in.
	 * <p/>
	 * <p/>
	...
	/**
	 * Compute the time of the next operation. Does not modify any state (unless
	 * the clock rolls backwards, in which case timers are reset).
	 *
	 * @param options to use for this computation.
	 * @return the wall clock time ({@link System#currentTimeMillis()}) when the
	 *         next operation should be attempted -- immediately, if the return
	 *         value is before the current time.
	 */
	public long getNextTimeMillis(Options options) {
		...
		// We do quite a bit of limiting to prevent a clock rollback from
		// totally
	...
	/**
	 * Return the last time the operation was attempted. Does not modify any
	 * state.
	 *
	 * @return the wall clock time when {@link #onSuccess()} or
	 *         {@link #onTransientError()} was last called.
	 */
	public long getLastAttemptTimeMillis() {
		return Math.max(mStorage.getLong(PREFIX + "lastSuccessTimeMillis", 0),
						mStorage.getLong(PREFIX + "lastErrorTimeMillis", 0));
	}

	/**
	 * Fetch a {@link SharedPreferences} property, but force it to be before a
	 * certain time, updating the value if necessary. This is to recover
	...
	private long getTimeBefore(String name, long max) {
		...
		if (time > max) {
			time = max;
			mStorage.edit().putLong(name, time).apply();
		}
		return time;
	}
	 ...
	 * scheduled time may be affected by error backoff logic and defined minimum
	 * intervals. Use {@link Long#MAX_VALUE} to disable triggering.
	 *
	...
	public void setTriggerTimeMillis(long millis) {
		mStorage.edit().putLong(PREFIX + "triggerTimeMillis", millis).apply();
	}

	/**
	 * Forbid any operations until after a certain (absolute) time. Limited by
	 * {@link #Options.maxMoratoriumMillis}.
	 *
	 * @param millis wall clock time ({@link System#currentTimeMillis()}) when
	 *               operations should be allowed again; 0 to remove moratorium
	 */
	public void setMoratoriumTimeMillis(long millis) {
		mStorage.edit().putLong(PREFIX + "moratoriumTimeMillis", millis)
				.putLong(PREFIX + "moratoriumSetTimeMillis", currentTimeMillis()).apply();
	}

	/**
	 * Forbid any operations until after a certain time, as specified in the
	 * format used by the HTTP "Retry-After" header. Limited by {@link #Options.maxMoratoriumMillis}
	 * .
	 *
	 ...
	 * {@link #getNextTimeMillis()} return {@link Long#MAX_VALUE}. Commonly used
	 * when data network availability goes up and down.
	 *
	 * @param enabled if operations can be performed
	 */
	public void setEnabledState(boolean enabled) {
		mStorage.edit().putBoolean(PREFIX + "enabledState", enabled).apply();
	}

	/**
	 * Report successful completion of an operation. Resets all error counters,
	 * clears any trigger directives, and records the success.
	 */
	public void onSuccess() {
		resetTransientError();
		resetPermanentError();
		mStorage.edit().remove(PREFIX + "errorCount").remove(PREFIX + "lastErrorTimeMillis")
				.remove(PREFIX + "permanentError").remove(PREFIX + "triggerTimeMillis")
				.putLong(PREFIX + "lastSuccessTimeMillis", currentTimeMillis()).apply();
	}

	/**
	 * Report a transient error (usually a network failure). Increments the
	 * error count and records the time of the latest error for backoff
	 * purposes.
	 */
	public void onTransientError() {
		...
		editor.putInt(PREFIX + "errorCount", mStorage.getInt(PREFIX + "errorCount", 0) + 1);
		editor.apply();
	}
	...
	public void resetTransientError() {
		mStorage.edit().remove(PREFIX + "errorCount").apply();
	}

	/**
	 * Report a permanent error that will not go away until further notice. No
	 * operation will be scheduled until {@link #resetPermanentError()} is
	 * called. Commonly used for authentication failures (which are reset when
	 * the accounts database is updated).
	 */
	public void onPermanentError() {
		mStorage.edit().putBoolean(PREFIX + "permanentError", true).apply();
	}
	...
	public void resetPermanentError() {
		mStorage.edit().remove(PREFIX + "permanentError").apply();
	}
	...
	public String toString() {
		...
		for (String key : new TreeSet<String>(mStorage.getAll().keySet())) { // Sort
			// keys
